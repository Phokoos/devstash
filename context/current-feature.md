...
- Seed data: demo user (demo@devstash.io, bcryptjs 12 rounds) plus 5 collections and 18 items — React Patterns, AI Workflows, DevOps, Terminal Commands, Design Resources; system item type names kept plural/capitalised to match the dashboard UI; the whole script is idempotent (upsert by stable id, connectOrCreate for tags and collection links) so `npm run db:seed` can be re-run at any time
